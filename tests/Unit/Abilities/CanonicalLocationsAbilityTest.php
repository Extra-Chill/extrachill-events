...
final class CanonicalLocationsAbilityTest extends WP_UnitTestCase {
	...
	protected function setUp(): void {
		...
		// (#890) — this test builds a parent/child term tree below, and a
		// non-hierarchical registration here would also silently poison any
		// other test relying on 'location' hierarchy that runs later in the
		// same managed PHPUnit process (taxonomy registrations live in
		...
		$usa            = self::factory()->term->create( array(
			'taxonomy' => 'location',
			'name'     => 'USA',
			'slug'     => 'usa',
		) );
		$south_carolina = self::factory()->term->create( array(
			'taxonomy' => 'location',
			'name'     => 'South Carolina',
			'slug'     => 'south-carolina',
			'parent'   => $usa,
		) );
		$texas          = self::factory()->term->create( array(
			'taxonomy' => 'location',
			'name'     => 'Texas',
			'slug'     => 'texas',
			'parent'   => $usa,
		) );
		$charleston_sc  = self::factory()->term->create( array(
			'taxonomy' => 'location',
			'name'     => 'Charleston',
			'slug'     => 'charleston-sc',
			'parent'   => $south_carolina,
		) );
	...
	public function test_search_returns_only_selectable_cities_and_restores_blog(): void {
		$result = extrachill_events_ability_locations( array(
			'mode'   => 'search',
			'search' => 'charleston',
		) );
	...
	public function test_missing_and_nonselectable_slugs_return_not_found(): void {
		$missing = extrachill_events_ability_locations( array(
			'mode' => 'resolve',
			'slug' => 'missing',
		) );
		$state   = extrachill_events_ability_locations( array(
			'mode' => 'resolve',
			'slug' => 'texas',
		) );
