...
Attachment creation is booking-scoped and idempotent. A MySQL advisory lock serializes each storage reference across claim creation, active-reference checks, and cleanup. Reference reads propagate database failures instead of treating uncertainty as zero references. The private object claim is marked `abandoned` if metadata validation, metadata insertion, or audit insertion fails; failed compensation is surfaced explicitly and abandoned claims become bounded provisional-cleanup candidates. Cross-booking reuse is disabled until a non-null canonical artist identity and current artist authority can both be proven; unresolved identity always fails closed.

Replacement creates the new reference first, then transactionally retires the previous metadata as `replaced` with its activity entry; failed replacement compensation leaves an explicit `abandoned` reference for recovery. Delete is logical and transactional with its activity entry. Every sensitive mutation locks current booking and venue-membership rows and reauthorizes inside its transaction. Physical cleanup transitions eligible references to recoverable `purging` state before provider retirement; provider retirement writes a tombstone before deleting bytes and can resume after partial deletion.

No download token or storage reference is written to booking activity. `BookingAttachmentService::cleanup()` has no default destructive policy: an operator must explicitly supply both approved retention days and a legal-hold callback. Interrupted provisional cleanup is available through the provider, but no destructive cleanup is scheduled until #336 and #317 approve retention, legal holds, backups, and operational ownership.

## Future Admission Seam

The local provider's `stage()` method accepts a server-controlled temporary source, filename, and purpose, then returns only an opaque reference. `attach()` accepts that reference plus explicit `anonymous`, `user`, `email`, or `system` attribution and an optional inbound reference. This supports future public inquiry and inbound-email adapters without teaching the booking domain about REST or mail transport. API #123 still owns multipart admission, Turnstile, provisional upload orchestration, mapping protected download requests into `download_descriptor()` plus `open_download_stream()`, byte-response transport, and request rate limiting. Until that protected integration exists, the handoff is intentionally not REST-exposed.
